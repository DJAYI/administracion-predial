...
public class TipoExpedienteController {
    ...
    public ResponseEntity<PropertyTypeResponse> create(@RequestBody ExpedientRequest request) {
        return ResponseEntity.ok(service.create(request));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'EJECUTOR_INTEGRAL')")
    public ResponseEntity<PropertyTypeResponse> update(@PathVariable Long id, @RequestBody ExpedientRequest request) {

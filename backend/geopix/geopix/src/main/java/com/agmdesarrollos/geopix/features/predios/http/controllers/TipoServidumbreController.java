...
@RestController
@RequestMapping("/api/v1/servidumbres")
...
public class TipoServidumbreController {
    ...
    public ResponseEntity<PropertyTypeResponse> create(@RequestBody EasementRequest request) {
        return ResponseEntity.ok(service.create(request));
    }

    @PutMapping("/{id}")
    @PreAuthorize("hasAnyRole('ADMIN', 'EJECUTOR_INTEGRAL')")
    public ResponseEntity<PropertyTypeResponse> update(@PathVariable Long id, @RequestBody EasementRequest request) {
